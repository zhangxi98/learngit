...
//         $arr['ww']                   = !empty($arr['ww']) ? $arr['ww'] : '';
        ...
        /* fulltext_search_add_START_www.6gyhdnet.com */
	$arr['fulltext_search']            = isset($arr['fulltext_search']) ? $arr['fulltext_search'] : 0;  
        /* fulltext_search_add_END_www.6gyhdnet.com */
 ...
 */
// 代码修改_start_derek20150129admin_goods  www.gyhdnet.com
function get_brand_list($t = false)
// 代码修改_end_derek20150129admin_goods  www.gyhdnet.com
{
...
{
/* 代码增加_虚拟团购_START  www.6gyhdnet.com */
    $add_date = gmtime();
...
/* 代码增加_虚拟团购_END  www.6gyhdnet.com */
...
/**
 *  清除手机指定后缀的模板缓存或编译文件
 *  wei2 增加 start by www.gyhdnet.com
 * @access  public
 * @param  bool       $is_cache  是否清除缓存还是清出编译文件
 * @param  string     $ext       需要删除的文件名，不包含后缀
 *
...
/* wei2 增加 end by www.6gyhdnet.com */
/**
 * 清除模版编译文件
 *
 * @access  public
...
/* wei2 修改 end by www.6gyhdnet.com */
/**
 * 页面上调用的js文件
 *
 * @access  public
...
function get_package_goods($package_id, $package_attr_id='')
{
	...
	if ($package_attr_id)
	{
	 ...
	 // 下面SQL语句增加两个字段，注意逗号 , g.goods_thumb, g.shop_price   By  www.gyhdnet.com
...
function Recordkeyword($word_www_68ecshop_com, $items = 0, $searchengine = 'ecshop')
{
	...
	if($r)
	{
		$items = intval($items) ;   //www.gyhdnet.com
