...
/* 代码增加_Start By www.gyhdnet.com  用户中心获取会员信息*/
...
function action_default ()
{
	...
	// 获取全局变量
    /* 代码增加 By  www.gyhdnet.com Start */
    $_CFG = $GLOBALS['_CFG'];
    /* 代码增加 By  www.gyhdnet.com End */
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	$db = $GLOBALS['db'];
	$ecs = $GLOBALS['ecs'];
	$user_id = $_SESSION['user_id'];
	/* 代码注释 by www.gyhdnet.com _star 路由前已调用会员信息，去掉重复调用*/
	$rank = get_rank_info();
	/* 	include_once (ROOT_PATH . 'includes/lib_clips.php');
	 if($rank = get_rank_info())
	 {
	...
	/* 代码注释 by www.gyhdnet.com _end */
	
	/* 代码增加2014-12-23 by www.gyhdnet.com _star */
	...
	/* 代码增加2014-12-23 by www.gyhdnet.com _end */
	/* 代码增加--cb--推荐分成-- by www.gyhdnet.com _star */
	$rn = $rank['rank_name'];
	$recomm = $db->getOne("SELECT is_recomm FROM " . $GLOBALS['ecs']->table('user_rank') . " WHERE rank_name= '$rn'");

    $smarty->assign('recomm', $recomm); // 获取当前用户是否是分成用户判断是否显示我的推荐
	/* 代码增加--cb--推荐分成-- by www.gyhdnet.com _end */
...
//	$order['shipping_status'] = $_LANG['ss'][$order['shipping_status']];
    /* 代码删除 By  www.gyhdnet.com End */
	/* 增值税发票_添加_START_www.6gyhdnet.com */
	/* 增值税发票收票地址 */
	...
	foreach($goods_list as $goods_key => $goods_val)
	{
		...
		/* 代码增加_start By www.gyhdnet.com _20150804 */
		...
		/* 代码增加_end By www.gyhdnet.com _20150804 */
...
/* 商品评价/晒单 增加 by www.6gyhdnet.com */
...
/* 余额额支付密码_添加_START_www.6gyhdnet.com */
...
/* 账户安全_end_www.6gyhdnet.com */
/* 余额额支付密码_添加_END_www.6gyhdnet.com */
