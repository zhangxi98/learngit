 ...
 * ECSHOP 购物流程
 * ============================================================================
 * * 版权所有 2008-2015 第八号网络科技，并保留所有权利。
 * 网站地址: http://www.zqisc.com;
 * ----------------------------------------------------------------------------
...
elseif ($_REQUEST['act']=='delAddress')
{
	...
	$consignee_list = get_consignee_list_zhouqimall();
...
elseif ($_REQUEST['act']=='saveAddress')
{
	...
	$address_zhouqimall = $json->decode($_POST['address']);
	$consignee = array(
            'address_id'    => empty($address_zhouqimall->address_id) ?  '0'  :   intval($address_zhouqimall->address_id),
            'consignee'     => empty($address_zhouqimall->consignee)  ? '' :   compile_str(trim($address_zhouqimall->consignee)),
            'country'       => empty($address_zhouqimall->country)    ? '' :   intval($address_zhouqimall->country),
            'province'      => empty($address_zhouqimall->province)   ? '' :   intval($address_zhouqimall->province),
            'city'          => empty($address_zhouqimall->city)       ? '' :   intval($address_zhouqimall->city),
            'district'      => empty($address_zhouqimall->district)   ? '' :   intval($address_zhouqimall->district),
            'email'         => empty($address_zhouqimall->email)      ? '' :   compile_str($address_zhouqimall->email),
            'address'       => empty($address_zhouqimall->address)    ? '' :   compile_str($address_zhouqimall->address),
            'zipcode'       => empty($address_zhouqimall->zipcode)    ? '' :   compile_str(make_semiangle(trim($address_zhouqimall->zipcode))),
            'tel'           => empty($address_zhouqimall->tel)        ? '' :   compile_str(make_semiangle(trim($address_zhouqimall->tel))),
            'mobile'        => empty($address_zhouqimall->mobile)     ? '' :   compile_str(make_semiangle(trim($address_zhouqimall->mobile))),
     );
	...
	$result = array('error' => 0, 'message' => '', 'content' => '', 'closediv'=>$address_zhouqimall->closediv);    
	$consignee_list = get_consignee_list_zhouqimall();
	$smarty->assign('consignee_list', $consignee_list);
	$result['content'] =  $smarty->fetch("library/address_list.lbi");
	...
	if ($address_zhouqimall->shipping_bian=='0' && $address_zhouqimall->address_id>0 && $address_zhouqimall->address_id==$_SESSION['flow_consignee']['address_id'])
	{
		$sql = "SELECT * ".
                    " FROM " . $GLOBALS['ecs']->table('user_address') . 
                    " WHERE  address_id = '".$address_zhouqimall->address_id."' ";
		$consignee = $GLOBALS['db']->getRow($sql);
		$_SESSION['flow_consignee'] = $consignee;
	}
	if ($address_zhouqimall->shipping_bian=='1' || ($address_zhouqimall->shipping_bian=='0' && $address_zhouqimall->address_id>0 && $address_zhouqimall->address_id==$_SESSION['flow_consignee']['address_id']))
	{
	$order = flow_order_info();
	$flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;
    $region            = array($consignee['country'], $consignee['province'], $consignee['city'], $consignee['district']);
...
function get_consignee_list_zhouqimall()
{
	$consignee = get_consignee($_SESSION['user_id']);
	$sql="SELECT * FROM " . $GLOBALS['ecs']->table('user_address') .
            " WHERE user_id = '". $_SESSION['user_id'] ."' order by address_id ";
	$consignee_list_zhouqimall = $GLOBALS['db']->getAll($sql);
	foreach ($consignee_list_zhouqimall  as $cons_key => $cons_val)
	{
				$consignee_list_zhouqimall[$cons_key]['address_short_name'] = $cons_val['consignee']."<br>";
				$consignee_list_zhouqimall[$cons_key]['address_short_name'] .=  get_region_info($cons_val['province'])."-";
				$consignee_list_zhouqimall[$cons_key]['address_short_name'] .=  get_region_info($cons_val['city'])."-";
				$consignee_list_zhouqimall[$cons_key]['address_short_name'] .=  get_region_info($cons_val['district'])."&nbsp;";
				$consignee_list_zhouqimall[$cons_key]['address_short_name'] .=  sub_str($cons_val['address'],16);
				$consignee_list_zhouqimall[$cons_key]['address_short_name'] .=  $cons_val['zipcode'] ? (",".$cons_val['zipcode']) : "";
				$consignee_list_zhouqimall[$cons_key]['address_short_name'] .=  "<br>".($cons_val['tel'] != '--' ? $cons_val['tel'] : $cons_val['mobile']);
				if ($consignee['address_id'] == $cons_val['address_id'])
				{
					$consignee_list_zhouqimall[$cons_key]['def_addr'] =1;
					$have_def_addr=1;
				}
	}
	if ( count($consignee_list_zhouqimall) && !$have_def_addr){ $consignee_list_zhouqimall[0]['def_addr'] =1; }
	return 	$consignee_list_zhouqimall;	
}
/* 代码增加_end  By www.zqisc.com  */
...
$consignee['region'] = $db->getOne($sql); 
    ...
    {	
		...
		foreach ($consignee_list_zhouqimall  as $cons_key => $cons_val)
		{
			...
			$consignee_list_zhouqimall[$cons_key]['address_short_name'] .=  sub_str($cons_val['address'],16);
			$consignee_list_zhouqimall[$cons_key]['address_short_name'] .=  $cons_val['zipcode'] ? (",".$cons_val['zipcode']) : "";
			$consignee_list_zhouqimall[$cons_key]['address_short_name'] .=  "<br>".$cons_val['tel'];
			if ($consignee['address_id'] == $cons_val['address_id'])
			{
				$consignee_list_zhouqimall[$cons_key]['def_addr'] =1;
				$have_def_addr=1;
   ...
   $smarty->assign('name_of_region',   array($_CFG['name_of_region_1'], $_CFG['name_of_region_2'], $_CFG['name_of_region_3'], $_CFG['name_of_region_4']));
   $smarty->assign('consignee_list', $consignee_list_zhouqimall);
   $smarty->assign('shop_province_list', get_regions(1, $_CFG['shop_country']));
...
elseif ($_REQUEST['step'] == 'done')
{
	...
	$order['defaultbank'] = $_POST['www_zqisc_com_bank'] ? trim($_POST['www_zqisc_com_bank']) : "";
...
/* 客户下订单时给商家发送短信提醒 palenggege添加 */
   ...
   'code'=>$order['order_sn']
    ...
    if ($order['order_amount'] > 0)
    {
        ...
        if ($payment[0] > 0 || $order['pay_id']==10)
	{
...
function flow_drop_cart_goods($id)
{
    ...
    if ($row)
    {
		$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";//添加 www.zqisc.com
        //如果是超值礼包
...
{
    /* 查询：购物车中所有不可以单独销售的配件 */
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";//添加 www.zqisc.com
